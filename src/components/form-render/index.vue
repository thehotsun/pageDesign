    ...
    <template v-for="(widget, index) in widgetList">
      <template v-if="'container' === widget.category">
        <component :is="getContainerWidgetName(widget)" :widget="widget" :key="widget.id" :parent-list="widgetList"
          :index-of-parent-list="index" :parent-widget="null" @updateOtherComp="updateOtherComp">
        </component>
      </template>
      <template v-else>
...
export default {
  ...
  data () {
    return {
      formJsonObj: buildDefaultFormJson(),
  ...
  mounted () {
    ...
    this.$on('updateOtherComp', (compNameList, params) => {
      ...
      compNameList.map(compName => {
        try {
          const component = this.widgetRefList[compName]
          component?.refreshData(params)
        } catch (error) {
          console.error(error);
        }
      })
