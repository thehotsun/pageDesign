<template>
  <container-item-wrapper :widget="widget">
    ...
    <div class="sub-form-container" v-show="!widget.options.hidden">
      <el-row class="header-row">
        <div v-if="leftActionColumn" class="action-header-column">
          <span class="action-label">{{ i18nt('render.hint.subFormAction') }}</span>
          <el-button v-if="!isReadMode" :disabled="widgetDisabled || actionDisabled || insertDisabled" round
            type="primary" size="mini" class="action-button" @click="addSubFormRow"
        ...
        <template v-for="(subWidget) in widget.widgetList">
          <div :key="subWidget.id + 'thc'" class="field-header-column" v-if="!subWidget.options.hidden"
      ...
      <el-row v-for="(subFormRowId, sfrIdx) in rowIdData" class="sub-form-row" :key="subFormRowId">
        <div v-if="leftActionColumn" class="sub-form-action-column hide-label">
          <div class="action-button-column">
            <el-button :disabled="widgetDisabled || actionDisabled || insertDisabled" circle type=""
              icon="el-icon-circle-plus-outline" @click="insertSubFormRow(sfrIdx)" v-show="!isReadMode"
              :title="i18nt('render.hint.insertSubFormRow')"></el-button>
            <el-button :disabled="widgetDisabled || actionDisabled || deleteDisabled" circle type="" icon="el-icon-delete"
              @click="deleteSubFormRow(sfrIdx)" v-show="!isReadMode"
        ...
        <template v-for="(subWidget, swIdx) in widget.widgetList">
          <div class="sub-form-table-column hide-label" :key="subWidget.id + 'tc' + subFormRowId"
            v-if="!subWidget.options.hidden" :style="{ width: subWidget.options.columnWidth }">
...
export default {
  ...
  methods: {
    ...
    setSubFormValues (subFormValues) {
      this.$set(this.globalModel.formModel, this.widget.options.name, subFormValues)
      this.initRowIdData(false)
      this.initFieldSchemaData()
