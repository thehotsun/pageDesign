<template>
	<container-item-wrapper v-show="!widget.options.hidden" :widget="widget">
		...
		<el-table ref="dataTable" :data="widget.options.tableData" :class="[customClass]" :height="tableHeight"
			:style="{ 'width': widget.options.tableWidth }" :border="widget.options.border"
			:show-summary="widget.options.showSummary" :row-key="tableRowKey"
			:tree-props="{ children: widget.options.childrenKey }" :size="widget.options.tableSize"
			:stripe="widget.options.stripe" :highlight-current-row="singleRowSelectFlag" :row-class-name="getRowClassName"
			:span-method="getSpanMethod" v-loading="loadingFlag" @current-change="handleCurrentChange"
			@selection-change="handleSelectionChange" @sort-change="handleSortChange" @header-click="handleHeaderClick"
			@row-click="handleRowClick" @row-dblclick="handleRowDoubleClick" @cell-click="handleCellClick"
			@cell-dblclick="handleCellDoubleClick" @select="handleRowSelect" @select-all="handleAllSelect"
			...
			<el-table-column v-if="widget.options.showCheckBox" type="selection" :width="selectionWidth"
				fixed="left"></el-table-column>

			<el-table-column v-for="(item, index) in widget.options.tableColumns" v-if="item.show !== false" :key="index"
				:prop="item.prop" :label="item.label" :sortable="item.sortable" :fixed="!item.fixed ? false : item.fixed"
				:align="item.align ? item.align : 'center'" :render-header="renderHeader" :formatter="formatterValue"
				:format="item.format" :show-overflow-tooltip="true" :min-width="item.width">
				<template slot-scope="scope">
					<template v-if="item.formatS === 'render' && !!item.render">
						<table-column-custom-render :row="scope.row" :column="item" :data-table-ref="getDataTableRef"
							:render="getColumnRender(scope.row, item)" />
					</template>
			...
			<template v-if="!!widget.options.showButtonsColumn">
				<el-table-column :fixed="buttonsColumnFixed" class-name="data-table-buttons-column" :align="'center'"
					:label="widget.options.buttonsColumnTitle" :width="widget.options.buttonsColumnWidth">
					<template #default="scope">
						<template v-for="(ob) in widget.options.operationButtons">
							<el-button v-show="showOperationButton(ob, scope.$index, scope.row)" :type="ob.type" :size="ob.size"
								:round="ob.round" :disabled="disableOperationButton(ob, scope.$index, scope.row)"
								@click="handleOperationButtonClick(ob.name, scope.$index, scope.row)"
								:class="['data-table-' + ob.name + '-button']">{{ getOperationButtonLabel(ob, scope.$index,
									scope.row) }}</el-button>
						</template>
					</template>
				</el-table-column>
		...
		<el-pagination v-if="widget.options.showPagination" :small="widget.options.smallPagination"
			:current-page="currentPage" :page-sizes="pageSizes" :page-size="pageSize" :layout="paginationLayout" :total="total"
...
<script>
import ContainerItemWrapper from '@/components/page-render/container-item/container-item-wrapper'
...
import FieldComponents from '@/components/page-render/form-widget/field-widget/index'
import refMixin from "@/components/page-render/refMixin"
import containerItemMixin from "@/components/page-render/container-item/containerItemMixin"
import TableColumnCustomRender from '@/components/page-render/table-column-custom-render'
