<template>
  <form-item-wrapper :designer="designer" :field="field" :rules="rules" :design-state="designState"
    ...
    <el-upload ref="fieldEditor" :disabled="field.options.disabled || isReadMode" :style="styleVariables"
      class="dynamicPseudoAfter" :action="realUploadURL" :headers="uploadHeaders" :data="uploadData"
      ...
      <template #file="{ file }">
        <div class="upload-file-list">
          ...
          <i class="el-icon-delete file-action" :title="i18nt('render.hint.removeFile')"
            v-if="!field.options.disabled && !isReadMode" @click="removeUploadFile(file.name, file.url, file.uid)"></i>
        </div>
      </template>
    </el-upload>
  </form-item-wrapper>
